    ...
    if input("Chcete provést nákup akcii?(a/n)") == "a":
        ...
        if (fond not in pocet_cyklu_list) == False:
            ...
            if (akt_cena_kup_akc * mnozstvi) < stav_uctu:
                ...
                c.execute ('UPDATE portfolio SET mnozstvi_akcii = ?, prumerna_cena_nakupu = ? WHERE majitel_portfolia = ? AND nazev_fondu = ?',(mnozstvi_nakup, prumerna_cena, jmeno, nazev_kupovaneho_fondu))
    ...
    if prazdne == 0:
        ...
        if (fond-1) < pocitadlo:
            ...
            if c.execute('SELECT * FROM portfolio WHERE majitel_portfolia = ? AND nazev_fondu = ?',(jmeno,nazev_prodavaneho_fondu)).fetchall()[0][2] >= mnozstvi:
                ...
                c.execute ('UPDATE portfolio SET mnozstvi_akcii = ? WHERE majitel_portfolia = ? AND nazev_fondu = ?',((vypis_portfolia[0][2] - mnozstvi), jmeno, nazev_prodavaneho_fondu))
                ...
                print(f"Prodal jste {round(mnozstvi,3)}ks akcií fondu {nazev_prodavaneho_fondu} za {round(mnozstvi * akt_cena_kup_akc,3)}Kč.")
                conn.close()
            else:
